...
class SmUCIWcomInstall {
	...
	private static function create_files() {
		...
		$files = array(
			array(
				'base'          => SM_UCIWCOM_FILE_MANAGING_DIR,
				'file'          => '.htaccess',
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( $file == SM_UCIWCOM_PLUGIN_BASENAME ) {
			$row_meta = array(
				
				'support' => '<a href="' . esc_url( apply_filters( 'sm_uci_support_url', admin_url() . 'admin.php?page=sm-uci-support' ) ) . '" title="' . esc_attr( __( 'Contact Support', 'import-woocommerce' ) ) . '" target="_blank">' . __( 'Support', 'import-woocommerce' ) . '</a>',
			);
			unset( $links['edit'] );
	...
	public static function showUpgradeNotification($currentPluginMetadata, $newPluginMetadata){
		...
		if (isset($newPluginMetadata->upgrade_notice) && strlen(trim($newPluginMetadata->upgrade_notice)) > 0){
			echo '<p style="background-color: #d54e21; padding: 10px; color: #f9f9f9; margin-top: 10px"><strong>'.esc_html__('Important Upgrade Notice:','import-woocommerce').'</strong> ';
